...
- 时间片大小为 2 （注：以下括号内表示当前时刻就绪队列中的进程、进程的剩余运行时间）
    ...
    此时 P2 排在队头，因此让 P2 上处理机。（注意： 2 时刻， P1 下处理机，同一时刻新进程 P2 到达，如果在题目中遇到这种情况，默认新到达的进程先进入就绪队列）
    ...
    - 11 时刻（P4(4)→P1(1)）：P4 时间片用完，重新回到就绪队列。P1 上处理机
    - 12 时刻（P4(4)）：P1 运行完，主动放弃处理机，此时就绪队列中只剩 P4 ， P4 上处理机
    - 14 时刻（P4(4)）：就绪队列为空因此让 P4 接着运行一个时间片
    - 16 时刻：所有进程运行结束
